...
'''
def logistic(x, x1, mu, s, A):
	xv = x/x1
	return A*(1/4.0/s) * np.cosh((xv-mu)/2.0/s)**(-2)

def boltzmann(x, x1,a, A):
	xv = x/x1
	return A*(2.0/np.pi)**0.5 * (xv**2 * np.exp(-xv**2/2.0/(a**2)))/a**3

def gumbel(x,x1, mu,b, A):
	xv = x/x1
	z = (xv-mu)/b
	return A*np.exp(-1.0*(z + np.exp(-z)))/b

log_init = (1, 53.7873757895, 20.3443103825, 6140956.95601)
bltz_init = (1, 37.5461720445, 5133049.50192)
limit_x = limit_y = 140
...
y_log = logistic(xvals, popt_log[0], popt_log[1], popt_log[2], popt_log[3])
...
plt.savefig('Logistic_axischange.png')
# plt.show()
...
y_b = boltzmann(xvals, popt_b[0], popt_b[1], popt_b[2])
...
plt.xlabel('Density')
plt.ylabel('Frequency')
plt.savefig('Boltzmann_axischange.png')
# plt.show()
...
y_g = gumbel(xvals, popt_g[0], popt_g[1], popt_g[2],popt_g[3])
...
plt.xlabel('Density')
plt.ylabel('Frequency')
plt.savefig('Gumbel_axischange.png')
# plt.show()
